...
Code for cleaning and organizing raw bird observation datasets sourced from the CAP LTER database. 

Very code was used to create datasets for a separate analysis of species richness and relative abundance of bird guilds (Haight et al. 2025) that was under development around the same time as this 2026 manuscript. Parts of of the processing steps in this R Markdown file (e.g., the 'truncating counts' part) were from that previous analysis can largely be ignored as irrelevant to this specific manuscript and its analysis. For context, please see: 
Haight, J. D., De Albuquerque, F. S., Bateman, H. L., Frazier, A. E., & Larson, K. L. (2025). Urbanization and climate drive long‐term bird community trends across a desert city ecosystem. Ecological Applications, 35(4), e70063. https://doi.org/10.1002/eap.70063


# Import data
Source for bird observation data
Lerman, S. B., Warren, P. S., Bateman, H., Katti, M., & Shochat, E. (2023). Point-count bird censusing: Long-term monitoring of bird abundance and diversity in central Arizona-Phoenix, ongoing since 2000 [Dataset]. Environmental Data Initiative. https://doi.org/10.6073/PASTA/4777D7F0A899F506D6D4F9B5D535BA09  

This includes a few different data objects to import:
1) Bird counts from each survey
2) Metadata for each point count survey (times, observers, etc.)
3) Bird monitoring locations
...
```{r read in the site location data}
...
# Bird Species Lists
From the bird observations, create lists of species that were observed.  
...
# Optional: Export Observation Data
```{r export the cleaned bird count data by survey, warning = FALSE}
# Count-level data were used in the process of filtering the spatiotemporal scope
...
These aren't used in the analyses.
