...
```{r bundle data}
  # Bundle data for BUGS
      str(bdata <- list(
        ...
        cov.psi = a.cov.occ,           # occupancy covariates, including '1' for the intercept
        ncov.occ = dim(a.cov.occ)[3],  # number of occupancy covariates, including the intercept
        cov.rho = a.cov.det,           # detection covariates, including '1' for the intercept
...
```{r set initial values}
      ...
      inits <- function(){ list(
        z = zst,
        # beta0.comm = rep(rnorm(1, -3.994, 0.05), bdata$n.season),
        beta0.comm = rnorm(1, -3.994, 0.05),
        beta.comm = c(
          rnorm(1, -3.994, 0.05),
          rnorm(1, -0.300, 0.05),
          rnorm(1,  0.380, 0.05),
...
```
```{r SUP FIGURE plot priors}
x <- seq(-10, 10,by=0.1)
priors<- rbind(
  data.frame(x = x, prob = dcauchy(x, location = 0, scale = 2.5), param = "cauchy"),
  data.frame(x = x, prob = dgamma(x, shape = 1, scale = 1), param = "gamma"),
  data.frame(x = x, prob = dnorm(x), param = "norm")
)
...
ggplot(aes(x = x, y = prob)) +
  ...
  geom_line(data = (priors %>% filter(param == "cauchy")), color = "red") +
  geom_line(data = (priors %>% filter(param == "gamma")), color = "blue") +
...
ggsave("~/Github/caplter-dynamicbirds/figures/appendix/priors.png",
       plot.prior,
       width = 4,
       height = 2,
       units = "in",
