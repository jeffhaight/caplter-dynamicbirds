...
# Count the species
  ...
  # Across all winter surveys:
      # 165 non-domestic species were only observed once
      obs %>% filter(season == "1_winter") %>% 
        filter(!code %in% c("DODU", "DOGO", "ISCA", "CHGO")) %>% filter(code %in% spp.known) %>%
        filter(det_season >= 1) %>% pull(code) %>% unique() %>% length()
      # 130 species were observed at least twice
      obs %>% filter(season == "1_winter") %>%
        filter(!code %in% c("DODU", "DOGO", "ISCA", "CHGO")) %>% filter(code %in% spp.known) %>%
         filter(det_season >= 2) %>% pull(code) %>% unique() %>% length()
  # Across all spring surveys:
      # 170 species were observed at least once
      obs %>% filter(season == "2_spring") %>%
        filter(!code %in% c("DODU", "DOGO", "ISCA", "CHGO")) %>% filter(code %in% spp.known) %>%
         filter(det_season >= 1) %>% pull(code) %>% unique() %>% length()
      # 139 were observed at least twice
      obs %>% filter(season == "2_spring") %>%
        filter(!code %in% c("DODU", "DOGO", "ISCA", "CHGO")) %>% filter(code %in% spp.known) %>%
         filter(det_season >= 2) %>% pull(code) %>% unique() %>% length()
